...
function loadScript(src, callback) {
  ...
  script.src =
    "Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress/src/" +
    src;
...
(function (ADL) {
  ...
  /*
   * Take grouped data and return median of values of entries in each group
   */
  CollectionSync.prototype.median = function (path, level) {
    if (!path) return this;
    ...
    // if it's grouped and deep checking is requested, descend
      ...
      for (var i = 0; i < data.length; i++) {
        data[i].data = new CollectionSync(data[i].data).median(
          path,
          level - 1
        ).contents;
      }
    ...
    for (var i = 0; i < data.length; i++) {
      var median = [];
      for (var j = 0; j < data[i].data.length; j++) {
        median.push(getVal(path, data[i].data[j]));
      }
      if (median.length > 0) median = median_(median);
      else median[0] = 0;
      // copy group id fields to new object
      var group = {};
      for (var j in data[i]) {
      ...
      // add median and sample
      group.median = median.length === 0 ? 0 : median;
      group.sample = group.data[0];
      ret.push(group);
    }
    this.contents = ret;
  ...
  CollectionAsync.prototype.median = proxyFactory("median");
