...
Das XapiProgress-Plugin sendet Daten an einen LRS (Learning Record Store). Ausgangspunkte können Daten aus (optional erweiterten) ILIAS-Events und Daten aus der Nutzung von (auch über den Seiten-Editor) eingebundenen H5P-Objekten sein.
...
Eine Rückreferenzierung von den im LRS gespeicherten Daten zum Anwender ist dann ausschließlich durch die ILIAS-Installation möglich.

Kurs-Nutzende können selbst entscheiden, ob Daten über das XapiProgress-Plugin an einen LRS gesendet werden. Das Plugin `Lp2LrsPrivacy` wird nicht mehr benötigt.
...
```
mkdir -p /[ILIAS_DOC_ROOT]/Customizing/global/plugins/Services/UIComponent/UserInterfaceHook
cd /[ILIAS_DOC_ROOT]/Customizing/global/plugins/Services/UIComponent/UserInterfaceHook
git clone -b release_8 --single-branch https://github.com/VerDatAs/XapiProgress.git XapiProgress
cd [ILIAS_DOC_ROOT]
...
In der Plugin-Übersicht finden Sie den Eintrag XapiProgress. Klicken Sie in der Zeile auf das Dropdown-Feld `Aktionen`. Im Dropdown-Menü klicken Sie dann auf `Installieren`, anschließend auf `Konfigurieren` zur Auswahl u.a. des LRS und abließend erneut im Dropdown-Menü auf `Aktivieren`.
...
Auf der Kommandozeile (Git-Bash), können Sie nachfolgende Befehle ausführen:

`cd /your ILIAS Installation/`

### Services/Tracking

`git apply Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress/patches/release_8/Services/Tracking/service.xml.patch`

`git apply Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress/patches/release_8/Services/Tracking/classes/class.ilChangeEvent.php.patch`

### Modules/LearningModule

`git apply Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress/patches/release_8/Modules/LearningModule/module.xml.patch`

`git apply Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress/patches/release_8/Modules/LearningModule/classes/class.ilLMTracker.php.patch`

### Modules/Test

`git apply Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress/patches/release_8/Modules/Test/module.xml.patch`

`git apply Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress/patches/release_8/Modules/Test/classes/class.ilTestOutputGUI.php.patch`

`git apply Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress/patches/release_8/Modules/Test/classes/class.ilTestPlayerAbstractGUI.php.patch`
...
Das Plugin XapiProgress reagiert auf bestimmte Events, die von ILIAS-Komponenten zu bestimmten Anlässen geworfen werden, indem aus den Event-Parametern xAPI-Statements generiert und diese an den in den Plugin-Einstellungen ausgewählten LRS (Learning Record Store) gesendet werden. 

## Übersicht unterstützter Events

Zum aktuellen Entwicklungsstand unterstützt das XapiProgress-Plugin folgende Events:
