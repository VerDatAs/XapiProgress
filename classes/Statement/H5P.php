...
class H5P extends AbstractStatement
{
    ...
    protected ?string $localContentName;
    protected ?string $localContentScore;
    protected ?string $subContentId;
    protected ?array $subContentTitle;
    protected ?array $dropInParam;
...
//todo check with cat
        if($object->getType() !== 'lm' && $object->getType() !== 'copa' && !($this->eventParam['queryParamH5PModule']['gotoLink'] ?? false)) {
            ...
            return $this->getH5PLink((string) $object->getRefId(), $object->getType()) . $stringIds;
        }
        ...
        if('illmpresentationgui' === strtolower($this->urlParams['baseClass'])) {
            ...
            return $this->getH5PLink($targetRef, 'pg') .  $stringIds;
    ...
    protected function getH5PLink(string $a_target, string $a_type): string
    {
        return str_replace('/Customizing/global/plugins/Services/UIComponent/UserInterfaceHook/XapiProgress','',ILIAS_HTTP_PATH)
            . '/goto.php?target=' . $a_type . '_' . $a_target . '&client_id=' . CLIENT_ID;
    }
    ...
    public function getObjectProperties(ilObject $object): array
    {
        ...
        if(in_array($object->getType(), ['lm', 'copa'])) {
            ...
            if($this->subContentId && $this->subContentTitle) {
                ...
                foreach ($this->subContentTitle as $subTitle) {
                    $name[] = ' — ' . $subTitle;//array_pop($this->subContentTitle);
                }

            }

            if (\ilXapiProgressConfig::getExtendedNames() && $this->localContentScore = $this->postParam()['result']['score']['scaled'] ?? null) {
...
//        $permaLink = $this->getObjectPermaLink($object, true, $this->localContentId, $this->subContentId);
    ...
    public function jsonSerialize(): array
    {
        ...
        $this->dropInObjectDefinition()->dropInResult();
    ...
    private function validateRequestKeys(array $param) : array
    {
        ...
        foreach ($param as $key => $value) {
            ...
            if(!($invalidString && $invalidInteger && $invalidUrl)) {
                ...
                $this->dic->logger()->root()->error('############ XapiProgress error received invalid H5P request key: ' . $key);
    ...
    private function castDropInObjectDefinition(array $param) : array
    {
        ...
        foreach($param as $key => $value) {
            ...
            if(!is_null($value)) {
                ...
                $this->dic->logger()->root()->log('############ XapiProgress error can not cast data type for received key: ' . $key);
        ...
        foreach($param as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                switch (true) {
                    ...
                    case in_array($key, ['success']):

                        if (filter_var($value, FILTER_SANITIZE_STRING) == "true") {
                            $value = true;
                        } else {
                            $value = false;
                        }
                    ...
                    case in_array($key, ['completion', 'duration']):
                    ...
                    case in_array($key, ['min', 'max', 'raw', 'scaled']):

                        $value = filter_var($value,FILTER_SANITIZE_NUMBER_FLOAT);
                        //(float) $value === filter_var($value,FILTER_SANITIZE_NUMBER_FLOAT) ? (float) $value : null;
                ...
                if (!is_null($value)) {
                    ...
                    $this->dic->logger()->root()->debug('############ XapiProgress can not cast data type for received key: ' . $key);
