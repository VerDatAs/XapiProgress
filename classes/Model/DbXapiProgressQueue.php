...
namespace ILIAS\Plugin\XapiProgress\Model;

use ILIAS\Plugin\XapiProgress\Xapi\Statement\XapiStatementBuilder;
use ILIAS\BackgroundTasks\Implementation\Bucket\State;
use ilDBConstants;
use function Sabre\HTTP\parseDate;

trait DbXapiProgressQueue
{
    protected static string $DB_TBL_NAME_EVENTS2LRS_QUEUE = 'xapip_queue';

    protected static array $DB_TBLS_IL_BT = ['bucket', 'task', 'value_to_task', 'value'];

    protected static int $STATE_DELETABLE = -1;

    protected static int $STATE_INIT = 0;

    protected static int $STATE_TASK_SCHEDULE = 1;

    protected static int $STATE_RUNNING = 11;

    protected static int $STATE_CRON_EXEC_1 = 2;

    protected static int $STATE_CRON_EXEC_2 = 3;

    protected static int $STATE_CRON_EXEC_3 = 4;

    protected static int $TIME_BEFORE_DELETE = 2;

    protected static int $STATE_CRON_FAILED = 5;
    ...
    public int $numQueueEntries = 0;


    /**
     * @throws \Exception
     */
    public function addInitialDbEntrty(array $param, bool $setQueueId = true) : ?int
    {
        $config = new \ilXapiProgressConfig();
        if(!in_array($param['event'], $config->getEvents())) { # $this->allowedEvents
            return null;
        }

        $param['queue_id'] = $this->dic->database()->nextId('xapip_queue');

        $param['date'] = $param['date'] ?? date('Y-m-d H:i:s');

        $this->statement = $param['statement'] ?? in_array($param['event'], \ilXapiProgressPlugin::getDefaultEvents()) ? '' : XapiStatementBuilder::getInstance($param['event'])->buildPostBody($param);
        ...
        $values = [
            ...
            'statement' => array('text', $this->statement)
        ];
